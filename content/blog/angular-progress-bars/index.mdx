...
<Demo gitHub="LukasMarx/angular-progress-bars"/>
...
<Box>Note, that the "viewBox" attribute has to be spelled with a capital b!</Box>
...
<Box>Note, that because we are using an angular data-binding for our width, we need to use "attr.width" instead of just "width". Otherwise angular will trow an error.</Box>

...
class Example extends React.Component {
  ...
  render(){
    return (<div ref={this.domRef} onClick={this.onDivClicked}>Example Div</div>);
  }
}
...
<Box>The element provided to the callback function is not a reference but the DOM element itself. That means that there is no "current" property.</Box>
...
class Example extends React.Component {
  ...
  render(){
    return (<div ref={this.setDivRef} onClick={this.onDivClicked}>Example Div</div>);
  }
}
...
function Example(props){
  ...
  return (<div ref={domRef} onClick={onDivClicked}>Example Div</div>);
}
